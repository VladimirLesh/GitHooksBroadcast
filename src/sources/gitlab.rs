...
pub fn verify(secret: &[u8], headers: &HeaderMap) -> Result<(), AppError> {
    ...
    if verify_shared_secret(secret, token.as_bytes()) { Ok(()) } else { Err(AppError::InvalidSignature) }
}
...
pub fn parse(headers: &HeaderMap, body: &[u8]) -> Result<Event, AppError> {
    ...
    let user_login = v["user_username"].as_str()
        .or_else(|| v["user"]["username"].as_str())
        .unwrap_or("");
    let actor = Actor { login: user_login.to_string(), url: None };
    ...
    let kind = match event_type.as_str() {
        "Push Hook" | "Tag Push Hook" => {
            ...
            EventKind::Push { branch, commits, compare_url: None, forced: false }
        }
        "Merge Request Hook" => {
    ...
    Ok(Event { source: SourceKind::Gitlab, repo, actor, kind })
}
