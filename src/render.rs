...
pub fn render(ev: &Event) -> Rendered {
    match &ev.kind {
        EventKind::Push { branch, commits, compare_url, forced } => {
            let n = commits.len();
            ...
            let mut html  = format!("<b>[{}]</b> <a href=\"{}\">{}</a> pushed <b>{}</b> {} to <code>{}</code>{}\n",
                                    esc_html(repo), esc_attr(ev.actor.url.as_deref().unwrap_or("")), esc_html(actor),
                                    n, plural, esc_html(branch), force);
            let mut md    = format!("**\\[{}]** {} pushed **{}** {} to `{}`{}\n",
                                    md_esc(repo), md_esc(actor), n, plural, md_esc(branch), force);

            for c in commits.iter().take(COMMITS_LIMIT) {
                let short = short_id(&c.id);
                let subject = subject(&c.message);
                plain.push_str(&format!("  {short} {} — {}\n", subject, c.author));
                html.push_str(&format!("• <a href=\"{}\"><code>{}</code></a> {} — <i>{}</i>\n",
                                       esc_attr(&c.url), short, esc_html(&subject), esc_html(&c.author)));
                md.push_str(&format!("- [`{}`]({}) {} — _{}_\n",
                                     short, c.url, md_esc(&subject), md_esc(&c.author)));
            }
            if n > COMMITS_LIMIT {
            ...
            Rendered { plain, html, markdown: md }
        }
        EventKind::PullRequest { action, number, title, url, base, head } => {
            let repo = &ev.repo.full_name;
            let actor = &ev.actor.login;
            let verb = action.verb();
            let plain = format!("[{repo}] {actor} {verb} PR #{number}: {title} ({head} → {base})\n{url}");
            ...
            let md    = format!("**\\[{}]** {} **{}** [PR #{}]({}): {} (`{}` → `{}`)",
                                md_esc(repo), md_esc(actor), verb, number, url, md_esc(title),
                                md_esc(head), md_esc(base));
            Rendered { plain, html, markdown: md }
        }
    }
}
